...
export default function Appointments() {
  ...
  const handleJoinWaitlist = async () => {
    try {
      const res = await api.addToWaitlist({
        patient_id: "demo-patient",
        specialty_required: specialty,
        urgency_level: urgency.toUpperCase(),
        symptoms: `Request for ${specialty} appointment (${urgency} priority)`
      });
      alert(res.message || "Added to AI Waitlist successfully!");
      fetchRecommendations();
    } catch (error: any) {
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">AI-optimized doctor schedule matching & queue management.</p>
        </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition shadow-sm shrink-0"
              >
                + Join AI Waitlist
              </button>
            </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 text-center">
            {booked ? (
              <div className="animate-in fade-in zoom-in">
                ...
                <p className="text-xs text-slate-500">Smart allocation complete. Check in-app notifications for details.</p>
                <button 
