...
export const api = {
  ...
  // Multi-Sensor Doctor Presence & AI Slot Allocation
  async sendRFIDEvent(rfidTagId: string, roomNumber: string = "OPD Cabin 101", action: string = "ENTER", readerId: string = "RFID-READER-MAIN") {
    const headers = { ...getAuthHeaders(), "Content-Type": "application/json" };
    const res = await fetch(`${API_URL}/presence/rfid`, {
      method: "POST",
      headers,
      body: JSON.stringify({ rfid_tag_id: rfidTagId, room_number: roomNumber, action, reader_id: readerId })
    });
    if (!res.ok) throw new Error("RFID sensor request failed");
    return res.json();
  },

  async sendFaceDetectionEvent(faceId: string, confidenceScore: number = 0.95, roomNumber: string = "OPD Cabin 101", cameraId: string = "CAM-DOOR-01") {
    const headers = { ...getAuthHeaders(), "Content-Type": "application/json" };
    const res = await fetch(`${API_URL}/presence/face-detection`, {
      method: "POST",
      headers,
      body: JSON.stringify({ face_id: faceId, confidence_score: confidenceScore, room_number: roomNumber, camera_id: cameraId })
    });
    if (!res.ok) throw new Error("Face detection event failed");
    return res.json();
  },

  async sendMobileProximityEvent(mobileDeviceId: string, distanceMeters: number = 1.2, beaconId: string = "BLE-BEACON-01") {
    const headers = { ...getAuthHeaders(), "Content-Type": "application/json" };
    const res = await fetch(`${API_URL}/presence/mobile-proximity`, {
      method: "POST",
      headers,
      body: JSON.stringify({ mobile_device_id: mobileDeviceId, distance_meters: distanceMeters, beacon_id: beaconId })
    });
    if (!res.ok) throw new Error("Mobile proximity event failed");
    return res.json();
  },

  async overrideDoctorStatus(doctorId: string, status: string, roomNumber?: string) {
    const headers = { ...getAuthHeaders(), "Content-Type": "application/json" };
    const res = await fetch(`${API_URL}/presence/override`, {
      method: "POST",
      headers,
      body: JSON.stringify({ doctor_id: doctorId, status, room_number: roomNumber || "OPD Cabin 101" })
    });
    if (!res.ok) throw new Error("Status override failed");
    return res.json();
  },

  async getDoctorPresences() {
    const headers = getAuthHeaders();
    const res = await fetch(`${API_URL}/presence/doctors`, { headers });
    if (!res.ok) throw new Error("Failed to fetch doctor presence statuses");
    return res.json();
  },

  async getPresenceSensorLogs(limit: number = 30) {
    const headers = getAuthHeaders();
    const res = await fetch(`${API_URL}/presence/logs?limit=${limit}`, { headers });
    if (!res.ok) throw new Error("Failed to fetch sensor logs");
    return res.json();
  },

  async addToWaitlist(data: { patient_id: string; doctor_id?: string; specialty_required?: string; urgency_level?: string; symptoms?: string; report_id?: string }) {
    const headers = { ...getAuthHeaders(), "Content-Type": "application/json" };
    const res = await fetch(`${API_URL}/presence/waitlist`, {
      method: "POST",
      headers,
      body: JSON.stringify(data)
    });
    if (!res.ok) throw new Error("Failed to add patient to waitlist");
    return res.json();
  },

  async getWaitlistQueue() {
    const headers = getAuthHeaders();
    const res = await fetch(`${API_URL}/presence/waitlist`, { headers });
    if (!res.ok) throw new Error("Failed to fetch waitlist queue");
    return res.json();
  },

  async triggerAIAllocation(doctorId: string) {
    const headers = { ...getAuthHeaders(), "Content-Type": "application/json" };
    const res = await fetch(`${API_URL}/presence/allocate`, {
      method: "POST",
      headers,
      body: JSON.stringify({ doctor_id: doctorId })
    });
    if (!res.ok) throw new Error("Failed to trigger AI slot allocation");
    return res.json();
  }
};
